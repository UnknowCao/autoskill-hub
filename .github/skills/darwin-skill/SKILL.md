---
name: darwin-skill
description: "Darwin Skill 2.0 (达尔文.skill 2.0): autonomous skill optimizer, v2.0 integrates Microsoft Research SkillLens (arXiv 2605.23899) 9-dim rubric + SkillOpt (arXiv 2605.23904) validation-gated design + human-in-the-loop checkpoints. Evaluates SKILL.md files using a 9-dimension rubric (structure + effectiveness + meta-skill blacklists), runs hill-climbing with git version control, uses runSubagent to launch independent judge agents for blind evaluation, validates improvements through test prompts with auto-break on diminishing returns, and can generate visual result cards on user request. Use when user mentions \"优化skill\", \"skill评分\", \"自动优化\", \"auto optimize\", \"skill质量检查\", \"达尔文\", \"darwin\", \"帮我改改skill\", \"skill怎么样\", \"提升skill质量\", \"skill review\", \"skill打分\"."
---
...
> 核心理念：**评估 → 改进 → 实测验证 → 人类确认 → 保留或回滚**（成果卡片仅在用户要求时生成，不主动执行）
...
3. **棘轮机制** — 只保留改进，自动回滚退步
4. **独立评分** — 评分用 `runSubagent` 工具启动子 agent，避免「自己改自己评」的偏差
...
2. 用 `runSubagent` 工具启动子 agent 执行：一个带skill跑，一个不带skill跑（baseline）
3. 对比输出质量，从以下角度打分：
   - 输出是否完成了用户意图？
   - 相比不带skill的baseline，质量提升明显吗？
   - 有没有skill引入的负面影响（过度冗余、跑偏、格式奇怪）？

若 `runSubagent` 工具不可用（超时/资源限制），退化为「干跑验证」：读完 skill 后模拟一个典型 prompt 的执行思路，判断流程是否合理；必须在 results.tsv 标注 `dry_run`。**dry_run 必须附推演记录**（针对该 prompt 的 expected 逐项推演：症状/流程判断），否则视为 `not_tested`（不计入 dim8 覆盖，反模式 #9）。**dry_run/not_tested 比例 > 30% → 评估失效警告且 dim8 强制降分**（来自本机 controlled study：dim8 实测维度权重 23%，无 full_test 验证时分数不可信；2026-07-27 真实案例：patent-forge 优化时把 7 个未测 prompt 虚标 dry_run，被用户当面戳穿）。
...
for each skill in 优化范围:
  ...
  # 效果评分（用 runSubagent 工具做，独立于主 agent）
  3. 对每个测试prompt，用 `runSubagent` 工具启动子 agent：
     - with_skill: prompt 里附上 SKILL.md 全文 + 测试prompt
     - baseline: prompt 里只给测试prompt（不带skill）
  4. 对比两组输出，打维度8的分
...
**如果 `runSubagent` 工具不可用**（超时、环境限制），维度8用干跑验证打分，标注 `dry_run`。不要因为跑不了测试就跳过这个维度——哪怕是模拟推演也比完全不看效果好。
...
**🔴 测试覆盖明细表（强制，反 #9 虚报）**：评分卡下方**必须**附每个测试 prompt 的实际测试方式 + 推演/输出摘要。一行汇总（如「2 full_test + 7 dry_run」）禁止——它会掩盖哪些 dry_run 实际没做推演。

```
| Prompt | 测试方式 | 推演/输出摘要（dry_run 必填，填不出 = not_tested）|
|---|---|---|
| P1 happy-path | full_test | baseline 5 条权项 vs with-skill 18 条权项；2 个 CHECKPOINT 标记 |
| P2 docx 路径 | full_test | ACIP 判定 + 9 节结构 + 术语表 8 项 + 公式/图编号 |
| P3 doc-type 歧义 | dry_run | 推演：Quick Decision 三分支→askQuestions；关键规则拦截发明主题词「权利要求」 |
| P4 机械领域 | dry_run | 推演：Phase 1 关键词命中「折叠」→机械类；附图须结构图非流程图；3 实施例跨 4 维度 |
| ... | ... | ... |
```

**dim8 打分规则**：not_tested 不计入覆盖；dry_run/not_tested 比例 > 30% → dim8 上限从 10 降到 8（反模式 #6/#9 联动）。
...
for each skill:
  ...
  while round < MAX_ROUNDS (默认3):
    ...
    # Step 4: 重新评估
    - 结构维度：主agent重新打分
    - 效果维度：用 `runSubagent` 工具启动独立子 agent 重跑测试prompt（关键！不能自己评自己）
...
| # | 反模式 | 为什么不要做 | 替代做法 |
|---|---|---|---|
| 1 | **同 context 自评自改** | 改完后立刻在同一 Claude session 打分，会有「我刚改的肯定更好」乐观偏差（SkillLens 实证 LLM-as-judge 准确率仅 46.4%）| 必须用 `runSubagent` 工具启动**独立子 agent** 评分，且至少 2 个 judge 共识才信 |
...
| 4 | **跳过 test-prompts 直接评分（含「半跳过」）** | 没有 test-prompts 的 dim8 是凭空打分，权重 23% 等于编造。「半跳过」= 标了 dry_run 但实际没做推演（只写「覆盖了」字样）—— 文字上绕过 #4 的「全跳」，本质等同未测 | Phase 0.5 强制设计 2-3 prompts；**dry_run 必须附推演记录**（症状/流程判断），否则 results.tsv 标 `not_tested` 且不计入 dim8 覆盖 |
| 5 | **轮内改多个维度** | 多变量同时变，分数升降无法归因到具体改动 | 每轮 1 个维度；相关簇（dim2/3/4）改其一时观察另两个是否跟涨 |
| 6 | **dry_run 比例 > 30%（含虚报）** | dim8 实测维度形同虚设，分数虚高（早期 40 次记录 67% dry_run，0 revert）。**虚报更恶劣**：把「没测」标成 dry_run，文字上凑覆盖率 | 强制至少 1 个真实 full_test；dry_run/not_tested 比例 > 30% → **dim8 强制降分**（不只是 ⚠️ 警告，而是实打实扣分，例如 dim8 上限从 10 降到 8） |
...
| 9 | **虚报测试覆盖率**（2026-07-27 真实案例） | 把「没测」标成 dry_run；dim8 权重 23% 但 full_test vs 虚标 dry_run 在打分时待遇几乎相同——成本-收益激励倒挂，理性 agent 会偷懒。根因：汇报格式只展示汇总行（「2 full_test + 7 dry_run」）掩盖细节，且「选哪些真跑」由主 agent 自决无独立监督 | (1) Phase 1 评分卡**必须**附测试覆盖明细表（每个 prompt 单列：测试方式 + 推演/输出摘要），dry_run 填不出摘要 = not_tested；(2) dim8 打分时 not_tested 不计入覆盖；(3) dry_run/not_tested > 30% 触发 dim8 强制降分（与 #6 联动） |
...
7. **评分独立性** — 效果维度必须用 `runSubagent` 工具启动子 agent 或至少干跑验证，不能在同一上下文里「改完直接评」
...
> **⚠️ 默认不主动执行。** 仅当用户明确要求（如「生成卡片」「出个成果图」）时才生成。优化流程本身不依赖卡片。

用户要求后，生成视觉成果卡片，截图保存为PNG。
...
- **不主动生成** — 优化完成后默认只展示文字版评分卡和 git diff，不自动生成 PNG
- **单skill卡片**：用户要求时，为指定skill的分数变化生成卡片
- **总览卡片**：用户要求时，为全部优化战绩生成汇总卡片

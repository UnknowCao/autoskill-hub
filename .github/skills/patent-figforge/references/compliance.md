...
| Unicode symbols (✓ ✗ ⚠ ⭐ ➜) render as their codepoint (e.g. "2713", "26A0") instead of the glyph — **only inside HTML-like labels** (`<TD>✓</TD>`, `<TABLE>...<B>⚠</B>...</TABLE>`); plain node labels usually OK | The resolved CJK font (SimHei on Windows) **lacks glyphs** for Unicode misc-symbols ranges (U+2600–U+27BF: ✓✗⚠⭐➜✅❌⛔). Graphviz's HTML-label font lookup has no fallback chain, so it emits the raw codepoint text. Distinct from row 3 (which fails on CJK chars themselves); here CJK renders fine, only the symbols break. | **Avoid symbols entirely in HTML labels** — use CJK text (`已覆盖`/`未覆盖`/`警告`) or ASCII (`[OK]`/`[X]`/`[!]`). Cell background color (palegreen/salmon/khaki) already conveys status, so the symbol is redundant. **Do NOT** try to fix by adding a symbol-capable font to the HTML-label fontname — Graphviz HTML labels honor only ONE font and you'd lose CJK. Real case: patent-forge fig4 traceability matrix shipped green cells reading "2713" instead of ✓; fixed by switching to `已覆盖` text labels. |
...
| 7 | Raster output (PNG) as the **primary** deliverable | Not editable; §1.84 prefers reproducible vector | Primary = **SVG** (editable, the filing vector source). **This skill does NOT emit PDF** — `_render()` only produces `svg` + `png`. If the filing office requires PDF, convert the SVG externally (`rsvg-convert -f pdf`, Inkscape CLI, or `dot -Tpdf` invoked outside the skill's API). PNG is fine for the `view_image` verification gate — graphviz renders it directly and reliably (verified <0.5s on stock Windows). |
...
| 9 | Omit the figure entirely and describe it in prose | A picture is required for system/method claims with multiple components | Always emit an SVG (filing source). PDF is NOT produced by this skill — convert externally if the filing office mandates PDF. |

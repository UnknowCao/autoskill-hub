...
| PNG render hangs indefinitely (SVG fine) | Graphviz Cairo/Pango PNG backend stalls on some Windows installs | Prefer **SVG** (canonical, editable). For PNG, render SVG→PNG via a separate converter (e.g. `cairosvg`), not via the Graphviz PNG backend. |
...
| 7 | Raster output (PNG) as the **primary** deliverable | Not editable; §1.84 prefers reproducible vector | Primary = **SVG** (editable) + **PDF** (filing). PNG only as preview. |

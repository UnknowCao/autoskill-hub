...
**关键禁止**: 圆角框、渐变、阴影、彩色填充。详见 [🚫 反例与黑名单](#-反例与黑名单-anti-patterns--do-not)。
...
## 🚫 反例与黑名单 (Anti-Patterns — DO NOT)

> 以下每一项都是真实提交中被驳回的原因。逐条对照检查。

| # | 反模式 | 严重度 | 为什么不要做 | 替代做法 |
|---|--------|:---:|------|------|
| 1 | 圆角框 `style=rounded` | 🔴 致命 | 非专业工程图风格，审查员一眼驳回 | 直角 `shape=box` |
| 2 | 线条过粗 >1.5pt | 🔴 致命 | 儿童画质感，USPTO 拒收 | 0.5–0.8pt |
| 3 | 箭头穿过其他框体 | 🔴 致命 | 致命制图错误 | 层间隙安全通道路由 |
| 4 | 编号写在框内 | 🔴 致命 | 不符合专利惯例 | 框外 + 细引线连接 |
| 5 | 彩色填充 | 🔴 致命 | B&W 要求，彩色需 petition | 白色填充 `fillcolor=white` |
| 6 | 图上加标题 | 🟡 违规 | 专利图只有底部图号 | 仅 `label="图1" labelloc="b"` |
| 7 | 引线交叉 | 🟡 违规 | 混乱、不专业 | 顺时针排列，`constraint=false` |
| 8 | 框宽高比 ≈ 1:1 | 🟡 违规 | 文字拥挤，不够宽 | 宽高比 ≥ 3:1 |
| 9 | 菱形高窄 | 🟡 违规 | 不美观，判断文字放不下 | 扁平菱形宽高比 ≈ 2:1 |
| 10 | 渐变/阴影/装饰 | 🟡 违规 | 违反 B&W 线图原则 | 纯黑线条，无装饰 |
| 11 | 线条交叉 (crossing lines) | 🟢 应避免 | 降低可读性 | 无法避免用跳线桥接 |
| 12 | 同一部件不同图编号不一致 | 🟢 应避免 | 审查员混淆 | 全篇文件中标记号一致 |

> 更多细节见：**Shape Types**（形状规范）、**线条规范**（线条禁止行为）、**提交前检查清单**（12项阻塞门）。

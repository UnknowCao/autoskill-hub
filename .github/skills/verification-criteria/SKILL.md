...
| C.2 完整性检查 | 双索引 | 每条需求 ≥ 1 VC | UNCOVERED/PARTIAL 清单 | 零VC → 🔴 UNCOVERED；大量 UNCOVERED(>30%) → 建议先执行 Workflow A |
| C.3 孤儿检测 | 双索引 | 每条VC必须关联已存在的需求 | ORPHAN/UNLINKED 清单 | 指向不存在需求 → 🔴 ORPHAN；大量 ORPHAN(>20%) → 建议先审计需求ID规范性 |

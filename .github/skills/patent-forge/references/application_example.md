# Patent Forge Examples

## Example: Focus Period Recommendation System

### User Input

> "我们有一个功能，根据用户的日历事件自动推荐适合专注工作的时间段，并且会根据用户的行为随时间学习优化推荐。"

### Generated Application Form

---

# 摘要

本发明公开一种基于认知转换成本评分和反馈驱动学习的专注时段推荐系统及方法，涉及时序日程管理技术领域。该系统包括时间间隙识别模块、可用性评分模块、用户行为学习模块和推荐生成模块，通过分析日历事件元数据计算各候选时段的认知适用性评分，并根据用户对推荐的隐式反馈持续优化评分模型，用以解决现有日程管理系统无法准确识别适合专注工作的时间段且无法自适应优化的技术问题，达到提供个性化专注时段推荐并随使用持续提升推荐准确度的技术效果。

摘要附图：图 1

# 背景技术

本发明涉及计算机实现的日程管理技术领域，特别涉及一种基于日历分析和用户行为建模的专注时段推荐系统及方法。

现代知识工作者面临日益严重的日程碎片化挑战。数字日历和日程安排工具的普及虽然促进了协作，但也导致了连续时间块的减少。研究表明，平均知识工作者每天的日程碎片化程度严重影响了深度工作的可能性。

现有的日历应用程序仅提供基本的事件可见性和空闲时间段识别功能。然而，这些系统通常采用二元可用性指标——将时间标记为"忙碌"或"空闲"——而不考虑可用时间段对于不同类型工作的定性适用性。一个技术上未被占用的时间段可能由于其与高认知负荷会议的时间接近性或持续时间不足，实际上并不适合进行专注工作。

一些日程管理系统尝试通过基于规则的方法解决这一限制，用户需要手动配置无会议时段的偏好。然而，这类系统需要显式的用户配置，无法适应变化的工作模式或个人生产力差异。

现有技术中缺乏能够自动识别和推荐适合专注工作的时间段、同时考虑周围已安排事件所建立的认知上下文并能适应个人用户模式的系统和方法。

# 检索分析

检索过程及结论
撰写提案前必须经过充分检索，充分检索有利于发明人自己理解发明创造的创意创新点

1. 检索关键词：日程管理 + 专注时段推荐 + 机器学习
2. 检索式：(calendar scheduling) AND (focus time recommendation)
3. 检索结果

|编号|申请号|专利名称|发明内容|
|-|-|-|-|
|1|US10234567B2|Calendar-based Task Scheduling System|基于日历数据识别空闲时间段并进行任务调度的系统|
|2|US9876543A1|Context-Aware Notification System|分析事件元数据进行上下文感知决策的通知系统|
|3|CN111222333A|智能日程推荐方法及装置|根据用户历史行为推荐日程安排的方法|

4. 检索结果分析
与本提案最接近的专利为US10234567B2，该专利要解决的技术问题是日历任务调度效率低的问题，所采用的技术方案是基于时间间隙识别进行任务分配，起到的效果是提高日程利用率。本提案要解决的技术问题是专注工作时段识别不精准的问题，所采用的技术方案是结合认知转换成本和用户行为学习的可用性评分系统，起到的效果是提供个性化的专注时段推荐并持续优化。本提案与该发明的相同点是都基于日历数据进行时间段分析，区别点是本提案引入认知转换成本计算和反馈驱动的自适应学习机制。

|最接近的现有技术专利申请号|与本提案的相同点|与本提案的区别点|
|-|-|-|
|US10234567B2|日历数据分析、时间间隙识别|认知转换成本评分、行为学习优化|
|US9876543A1|事件元数据分析、上下文感知|专注时段推荐、可用性评分算法|
|CN111222333A|用户行为学习、日程推荐|隐式反馈机制、认知负荷建模|

# 发明内容

1. 本发明要解决上述技术问题中最核心的技术问题是：现有日程管理系统无法准确识别适合专注工作的时间段，且无法根据个人用户模式进行自适应优化。

2. 从宏观上看，解决技术问题的技术方案可以概括提炼为：一种基于认知转换成本评分和反馈驱动学习的专注时段推荐系统，包括时间间隙识别模块、可用性评分模块、用户行为学习模块和推荐生成模块，通过分析日历事件元数据计算各候选时段的认知适用性评分，并根据用户对推荐的隐式反馈持续优化评分模型。

3. 技术方案带来的有益效果体现在：
   - 通过认知转换成本建模，推荐质量显著提升（基于逻辑推理：考虑相邻事件的认知影响可避免在高压会议后立即推荐专注时段）
   - 通过隐式反馈学习，推荐准确度随使用时间持续提高（用户无需显式配置即可获得个性化推荐）
   - 减少用户在日程管理上的认知负荷（量化指标：用户决策次数可降低）

# 权利要求书

1. 一种专注时段推荐方法，其特征在于，包括：
   获取用户的日历事件数据，所述日历事件数据包括事件开始时间、结束时间和事件元数据；
   其特征在于：
   基于所述日历事件数据识别候选专注时段；
   对每个候选时段计算可用性评分，所述可用性评分基于所述候选时段的持续时间和相邻事件的认知转换成本，其中所述认知转换成本依据相邻事件的事件类型分类权重确定；
   按可用性评分由高到低排列候选时段并生成推荐结果；
   采集用户对所述推荐结果的交互行为作为隐式反馈，并基于所述隐式反馈更新所述可用性评分的参数。

2. 根据权利要求 1 所述的方法，其特征在于，所述相邻事件的事件类型包括高注意力事件、协作事件和行政事件，每种事件类型被分配不同的认知转换成本权重。

3. 根据权利要求 1 所述的方法，其特征在于，所述隐式反馈包括用户对推荐时段的接受、拒绝或修改行为，所述参数更新采用梯度下降法基于所述隐式反馈调整评分模型权重。

4. 根据权利要求 1 所述的方法，其特征在于，还包括触发步骤，所述触发步骤选自以下至少一种：每日固定时间点的时间触发、响应于日历事件创建或修改的事件触发、反馈累积量达阈值的阈值触发、以及响应于用户查询请求的用户触发。

5. 根据权利要求 1 所述的方法，其特征在于，所述可用性评分进一步依据所述候选时段与最近一次已安排事件之间的时间间隔确定，时间间隔越短认知转换成本越高。

6. 根据权利要求 1 所述的方法，其特征在于，所述推荐结果按可用性评分由高到低选取前 N 个候选时段呈现，其中 N 为可配置参数且默认值为 3。

7. 根据权利要求 1 所述的方法，其特征在于，所述认知转换成本权重在冷启动阶段采用预设默认值，并在隐式反馈累积量超过冷启动阈值后切换为基于用户历史行为学习得到的个性化权重。

8. 一种专注时段推荐系统，其特征在于，包括：
   日历数据获取模块，配置为从日历应用读取日历事件数据；
   时间间隙识别模块，配置为基于所述日历事件数据识别候选专注时段；
   可用性评分模块，配置为对每个候选时段计算可用性评分，所述评分基于时段持续时间和相邻事件的认知转换成本；
   偏好学习模块，配置为采集用户对推荐时段的交互行为作为隐式反馈，并基于所述隐式反馈更新评分参数；
   推荐呈现模块，配置为将可用性评分最高的候选时段呈现于用户界面。

9. 根据权利要求 8 所述的系统，其特征在于，所述可用性评分模块进一步配置为将相邻事件分类为高注意力事件、协作事件或行政事件，并为每类事件分配不同的认知转换成本权重。

10. 根据权利要求 8 所述的系统，其特征在于，所述偏好学习模块采用逻辑回归模型或神经网络模型实现，模型输入包括候选时段属性向量和上下文特征向量，模型输出为可用性评分。

11. 根据权利要求 8 所述的系统，其特征在于，所述系统部署于移动终端设备上，所有模块在本地执行，日历数据不离开所述移动终端设备。

12. 根据权利要求 8 所述的系统，其特征在于，所述系统采用云端处理架构，包括客户端应用和远程处理系统，所述客户端应用配置为采集日历数据并上传，所述远程处理系统配置为基于多个用户的聚合数据训练通用评分模型并基于单一用户的反馈微调所述通用模型。

13. 根据权利要求 8 所述的系统，其特征在于，所述推荐呈现模块进一步配置为响应于混合触发条件生成推荐，所述混合触发条件包括时间触发、事件触发、阈值触发和用户触发中的至少两种。

14. 一种计算机可读存储介质，其上存储有计算机程序，其特征在于，该程序被处理器执行时实现权利要求 1 至 7 中任一项所述方法的步骤。

# 说明书附图

## 附图说明

- 图 1 为本发明实施例提供的专注时段推荐系统的整体架构示意图
- 图 2 为本发明实施例提供的专注时段推荐方法的核心流程图
- 图 3 为本发明实施例提供的可用性评分模块内部结构示意图
- 图 4 为本发明实施例提供的云端-边缘混合部署架构对比示意图

> 参考标号说明：10-日历数据获取模块；20-时间间隙识别模块；30-可用性评分模块；31-事件分类子模块；32-认知转换成本计算子模块；33-时段评分子模块；40-偏好学习模块；50-推荐呈现模块；60-客户端应用；70-远程处理系统。
>
> 附图由 `patent-figforge` skill 生成 SVG/PNG；正式申请需替换为专利制图 / Visio (.vsd) 原图。

# 具体实施方式

## 实施例一：移动终端设备上的本地处理方案

在本实施例中，所述系统部署于移动终端设备上，包括：

一种专注时段推荐装置，包括：
- 日历数据获取模块，配置为从本地日历应用读取日历事件数据，所述日历事件数据包括事件开始时间、结束时间和事件元数据；
- 时间间隙识别模块，配置为基于所述日历事件数据识别候选专注时段；
- 可用性评分模块，配置为对每个候选时段计算可用性评分，所述评分基于时段持续时间和相邻事件的认知转换成本；
- 偏好学习模块，配置为监测用户对推荐时段的交互行为，并基于所述交互行为更新评分参数；
- 推荐呈现模块，配置为将排名最高的候选时段呈现于用户界面。

所述认知转换成本的计算方法包括：将相邻事件分类为高注意力事件、协作事件或行政事件，并为每类事件分配不同的转换成本权重。

## 实施例二：云端处理与边缘推送方案

在本实施例中，所述系统采用云端处理架构：

- 客户端应用配置为采集日历数据并上传至远程处理系统；
- 远程处理系统包括：
  - 数据聚合模块，配置为接收并存储多个用户的日历数据；
  - 模型训练模块，配置为基于聚合数据训练通用评分模型；
  - 个性化模块，配置为根据单一用户的反馈数据微调所述通用模型；
  - 推荐计算模块，配置为生成个性化推荐并推送至客户端。

本实施例的优势在于可利用跨用户数据提升模型泛化能力，同时通过边缘推送减少客户端计算负荷。

## 实施例三：混合触发机制方案

在本实施例中，所述系统采用多种触发条件：

- 时间触发：每日固定时间点生成当日推荐；
- 事件触发：响应于新日历事件的创建或修改，重新计算受影响时段的评分；
- 阈值触发：当累积的用户反馈数量达到预设阈值时，触发模型参数更新；
- 用户触发：响应于用户的显式查询请求，即时生成推荐。

所述混合触发机制使系统能够在响应性和资源效率之间取得平衡。

# 其他

## 1. 关键创新点

- 创新点一：引入"认知转换成本"作为可计算的评分因子，将相邻事件的类型信息纳入时段可用性评估，区别于现有技术的二元可用性判断
- 创新点二：采用隐式反馈机制（接受/拒绝/修改推荐）进行持续学习，无需用户显式配置偏好，降低使用门槛
- 创新点三：评分模型同时考虑时段本身属性和上下文因素，实现多维度的可用性评估

## 2. 本技术方案潜在的替代方案

- 替代方案一：基于生理数据（如可穿戴设备采集的心率、睡眠质量）辅助判断用户的专注能力峰值时段，可与本方案结合形成关联申请
- 替代方案二：采用显式规则配置替代机器学习模型，用户可手动设定事件类型与转换成本的映射关系，适用于偏好确定性的用户群体

## 3. 本技术方案的缺陷

- 潜在缺陷一：认知转换成本的计算依赖于事件类型的准确分类，若用户未规范填写事件信息，可能影响评分准确性
- 潜在缺陷二：隐式反馈学习需要一定的交互数据积累才能达到较好的个性化效果，新用户的冷启动期推荐质量可能受限

# IPC 分类号

- G06Q 10/06（资源分配、调度，主分类号）
- G06F 16/21（基于日历事件的数据处理）
- CPC 对应：G06Q 10/0631、G06F 16/2158

# 现有技术文献清单

| 序号 | 公开号 | 标题 | 优先权日 | 与技术方案的关系 | 相关性等级 |
|-|-|-|-|-|-|
| 1 | US10234567B2 | Calendar-based Task Scheduling System | 2017-03-15 | 最接近现有技术：均基于日历数据分析时间间隙；区别在于本发明引入认知转换成本评分与隐式反馈学习 | ★★★（最接近）|
| 2 | US9876543A1 | Context-Aware Notification System | 2016-08-22 | 同样分析事件元数据做上下文感知决策；区别在于本发明专注时段推荐与可用性评分算法 | ★★ |
| 3 | CN111222333A | 智能日程推荐方法及装置 | 2019-11-08 | 同样基于用户行为学习做日程推荐；区别在于本发明采用隐式反馈与认知负荷建模 | ★★ |

> 本清单依据 Phase 2 检索审计日志（查询发送数 3 / 专利收到数 12 / 专利引用数 3）生成，供代理师与审查员核查检索充分性。检索覆盖 IPC G06Q 10/06 + G06F 16/21 + CPC 二次检索。

...
> **用途**: 此文件是 `patent-forge` skill 的模板分发入口。所有第三方公司模板在此注册后即可通过 `--template` 参数调用。
...
用户提到要生成专利文档
       ...
       │     ├─ 是 → 检查是否在下方"代理机构映射表"中
       │     │     ├─ 在表中 → 使用对应 template
       │     │     └─ 不在表中 → 回退 `acip` 通用交底书模板 + 告知用户：
       │     │           "「[agency]」模板未注册，已使用 ACIP 通用交底书模板。
       │     │             如需专属模板，请将 .docx 放入 assets/raw_templates/ 后注册。"
       │     └─ 否 ↓
       │
       ├─ 文档用途明确？
...
| _（其他代理机构，如三环/中科/超凡等）_ | _自动回退 `acip` + 告知用户_ |
...
```
c:\AI\.github\skills\patent-forge\assets\raw_templates\<公司简称>_<模板类型>.<原扩展名>
```

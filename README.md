...
| 📄 [**x2md**](./skills/x2md/) | Cross-cutting (toolchain / SUP.8) | **SUP.8** Configuration Mgmt · **SUP.10** · document work-product intake | Convert any office/document file (DOCX / PDF / PPTX / XLSX / HTML / CSV / JSON / …) to clean, LLM-ready Markdown — goes beyond upstream markitdown 0.1.7 with: auto XLSX-formula eval (=A+B → 实数, 非 NaN), formula-escaping fix ($...$ 内 \*\_^ 修复), two-stage table auto-repair (vertical_merge D2 + nested), encrypted-file decryption (Windows CredUI / keyring, 密码不过 chat); single-file / parallel batch / size-aware resumable batch. 14-incident 反模式库 + 可复录 showcase (formula 19→0, table sidecar 35 行, xlsx 15/15). |

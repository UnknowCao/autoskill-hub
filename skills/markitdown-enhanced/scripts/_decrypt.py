...
Credentials are stored under service 'markitdown-enhanced', name = file stem.
...
def _get_credential(name: str) -> str | None:
    ...
    try:
        import keyring
        return keyring.get_password("markitdown-enhanced", name)
    except ImportError:
        return None
    except Exception:
        return None
...
def set_credential(name: str, password: str) -> None:
    """Store a password in the credential store under service 'markitdown-enhanced'.
    ...
    """
    import keyring
    keyring.set_password("markitdown-enhanced", name, password)
...
def _delete_credential(name: str) -> None:
    ...
    try:
        import keyring
        keyring.delete_password("markitdown-enhanced", name)
    except Exception:
...
def prompt_and_get_password(file_path: Path) -> tuple[str | None, bool]:
    ...
    target = f"markitdown-enhanced: {file_path.name}"
    ...
    uiinfo = {
        "MessageText": message,
        "CaptionText": "加密文件解密 - markitdown-enhanced",
    }
    try:

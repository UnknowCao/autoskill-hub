...
| 5 | **使用产品名 / 品牌名 / UI 术语**（如 iPhone、Google、点击按钮） | 替换为通用设备术语 / 标准专利表述，详见 [`language_conventions.md`](./language_conventions.md) | 不符合中国专利法用语规范 → 形式审查驳回 |
...
| 22 | **跨发明主题共用同一个 `04-diagrams/` 目录**（或在扁平 `patent-forge-output/04-diagrams/` 下堆放多份交底书的附图） | 每个发明主题必须创建独立的顶层主题目录 `patent-forge-output/<TopicSlug>/`，6 级子目录（含 `04-diagrams/`）全部位于该主题目录下。调用 `fill_acip_template.py` 时 `--figures-dir` 必须显式传 `<TopicSlug>/04-diagrams/` 绝对路径，不依赖脚本默认发现逻辑。详见 `shared_workflow.md` § Output Layout + § Phase -1 主题目录创建步骤 | 脚本 `_discover_figures()` 按文件名前缀 `fig<N>_*.png` 匹配、不校验内容归属 → 上一份交底书的遗留附图（如图4）被错误嵌入新文档 → 用户直到打开 .docx 才发现"图X 是别的内容" → 文档作废重做 |
...
- CNIPA.AI 检索/撰写边界（#19）→ `patent_search_apis.md` § CNIPA.AI

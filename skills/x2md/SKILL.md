...
> [`assets/tasks/x2md-dyn.jsonc`](assets/tasks/x2md-dyn.jsonc)):
> 1. Read the template, replace 5 placeholders: `<LABEL>`, `<SOURCE>`, `<OUTDIR>`, `<WORKERS>`, `<TIMEOUT_MULT>`.
>    List-mode source: also delete `"--recursive"`. `heavy-max=3` is frozen.
> 2. `create_and_run_task` with the filled template (workspaceFolder = repo root).
> 3. Monitor via `get_task_output` — prints `[N/M] ...` progress every 10 files.
> 4. **Resume**: relaunch with new task label. Existing `.md` files auto-skipped.
...
| 14 | **Ship the default size→timeout bands to a much slower host without tuning** | Bands calibrated on a fast host; on a slow one markitdown's per-subprocess format import (mammoth/openpyxl/python-pptx) makes a small `.docx` take ~30s → the `<0.5MB→8s` band kills office files mid-cold-start (not hung, just slow import). Grows fail-log with false positives. | Lower `--workers`, rely on the automatic `office_floor=40s`, or pass `--timeout-mult 2`. **Full lever table + PDF-exclusion rationale: see "Slow-machine angle" above.** |
...
### Password Security
- Passwords stored via Python `keyring` (service: `x2md`, name: file stem).
...
>   - some formulas unresolved → `XLSX formula eval: 13/15 cells resolved,
>     2 unresolved → will emit NaN: sheet1!B6, sheet1!C6`
>   - clean → `XLSX formula eval: 15/15 cells resolved`
...
>   (see below), because downstream output will contain `NaN`.
...
| `generate_schematic_ai.py` | AI schematic generation (**optional, requires `OPENROUTER_API_KEY`** — not part of the x2md conversion pipeline; unrelated to file→Markdown; kept as a legacy utility) |

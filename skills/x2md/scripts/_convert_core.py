r"""x2md single-file conversion entry point.
...
def _maybe_eval_xlsx(raw: bytes, suffix: str) -> tuple[bytes, str]:
    """Pre-evaluate XLSX formulas so markitdown reads real values, not NaN.

    Returns ``(new_bytes, warning_line)``. ``warning_line`` is "" when clean,
    or a single stdout-ready line describing unresolved formula cells (the
    agent uses this to decide whether to STOP per SKILL.md's C-4 policy —
    the script itself never stops).

    Best-effort: returns ``(raw, "")`` if the file is not .xlsx, has no
    formulas, or the `formulas` library is not available. Never raises — if
    anything fails, the original bytes come back untouched with "".
    """
    if suffix.lower() not in (".xlsx",):
        return raw, ""
    try:
        from _xlsx_formula_eval import evaluate_xlsx_with_report
        new_bytes, rep = evaluate_xlsx_with_report(raw)
        return new_bytes, rep.one_line()
    except Exception:
        return raw, ""
...
def convert_file(input_path: Path, output_path: Path | None = None,
    ...
    if _is_encrypted(input_path):
        ...
        if buf is None:
            if not allow_prompt:
                report.append(
                    ...
                    f" python -c \"import keyring; keyring.set_password('x2md','{disp_stem}','<pw>')\"")
            else:
        ...
        try:
            ...
            raw, xlsx_warn = _maybe_eval_xlsx(decrypted_bytes, input_path.suffix)
            if xlsx_warn:
                report.append(f"- {xlsx_warn}")
        ...
        eval_bytes, xlsx_warn = _maybe_eval_xlsx(raw_bytes, input_path.suffix)
        if xlsx_warn:
            report.append(f"- {xlsx_warn}")
...
def main():
    ap = argparse.ArgumentParser(description="x2md single-file converter")
